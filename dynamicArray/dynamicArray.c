...
/* 动态数组的初始化 */
int dynamicArrayInit(dynamicArray *pArray, int capacity)//传入数组，数组的容量大小
{
    if (pArray == NULL)
    {
        return NULL_PTR;
...
int dynamicArrayAppointPosInsertData(dynamicArray *pArray, int pos, ELEMENTTYPE val)
{
    ...
    {
        pArray->data[idx + 1] = pArray->data[idx];//此种方法是从前往后移，前一个数会将后一个覆盖，当后一个数再向后移动时都是前一个数的值，因此采取从最后开始移
    }
#endif
    /* 找到对应的值 写入到数组中 */
    pArray->data[pos] = val;
